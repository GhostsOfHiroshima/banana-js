    ...
    es6.propertyValueGetter,
        ...
        if (host.type === 'ObjectExpression') {
            const methods = [
                    ...
                    (propertyName, host) => {
                    return node_1.parent(host)
                        .chain(p => {
                        if (p.type === 'VariableDeclarator' && identifier_1.isVariable(p.id)) {
                            let values = identifier_1.occurences(p.id)
                                .filter(o => node_1.parent(o)
                                .chain(node_1.parent)
                                ...
                                .or_else(false))
                                .map(o => node_1.parent(o).chain(node_1.parent).map((p) => p.right));
