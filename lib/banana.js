...
export default {
  ...
  parseCode(editor) {
      ...
      .if_ok(ast => this.tokensIn[editor.id] = descendants(ast).filter(isToken))
      .if_ok(_ => this.markerManagerIn[editor.id].clear(editor, 'error'))
      .if_error(error => this.markerManagerIn[editor.id].set(editor, 'error', [parseErrorToRange(editor, error)]));
  },
  ...
  activate(state) {
    ...
    atom.workspace.observeTextEditors(editor => {
        ...
        editor.onDidDestroy(event => {
            delete this.lastWord[editor.id];
        ...
        editor.onDidChangeCursorPosition(event => {
            ...
            if (word.match(/^\w+$/)) {
                if (word !== this.lastWord[editor.id]) {
                    this.lastWord[editor.id] = word;
                    markerManager.set(editor, 'normal', ranges(editor.getText(), word, this.tokensIn[editor.id]));
                }
            } else {
                this.lastWord[editor.id] = null;
