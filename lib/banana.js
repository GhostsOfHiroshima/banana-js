...
export default {
  ...
  activate(state) {
    ...
    atom.workspace.observeTextEditors(editor => {
        ...
        .if_ok(_ => this.markerManagerInEditor[editor.id].clear(editor, 'error'))
        .if_error(error => this.markerManagerInEditor[editor.id].set(editor, 'error', [parseErrorToRange(editor, error)]));
        ...
        editor.onDidChangeCursorPosition(event => {
            if (!this.enabled || editor.getSelectedText() || this.editing) {
                if (! this.enabled) {
                    this.markerManagerInEditor[editor.id].clearAll(editor);
                }
                return;
            }
            ...
            if (word.match(/^\w+$/)) {
                if (word !== this.lastWord) {
                    this.lastWord = word;
                    markerManager.set(editor, 'normal', ranges(editor.getText(), word, this.identifiersInEditor[editor.id]));
                }
            } else {
                this.lastWord = null;
